...
保留 QQ001–QQ360 / TQ361–TQ600 色号体系。2026-09-26 使用最新 5 张实拍色卡重新采样完整 600 色；此前樱花色值不再沿用。前 3 张为 75D×2（QQ001–QQ360），后 2 张为 50D×2（TQ361–TQ600）。
...
色值取自每个线穗主体中央区域的稳健中位采样，属于实拍视觉参考值，不等同于标准仪器 Lab 测色。 2026-09-26 另外使用近照对厂家色号 **101 / 102 / 1786** 三个色位进行了局部颜色校准，其 AL 内部编号与其它数据均保持不变。
...
`palette.html` 直接读取上述三套内置色卡数据，并提供：
- 完整色块网格；
- 色号 / alias / 编号区间搜索；
- 原始顺序、色相、明度、饱和度排序；
- 单色详情（HEX / RGB / Lab / 来源位置）；
- 同色卡相近色推荐。

因此色卡展示和自动匹配共用同一份数据，不维护第二份色值副本。
